...
@onready var _contexts:Container = %Contexts
...
func _process(delta):
    ...
    for context in GUIDE._active_contexts:
        var priority = GUIDE._active_contexts[context]
        var label := _get_label(_contexts, index)
        label.text = "[%s] %s" % [priority, context._editor_name()]

        index += 1

    # Clean out all labels we don't need anymore
    _cleanup(_contexts, index)
